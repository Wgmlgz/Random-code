...
#define elif else if
#define ins .insert
template<typename T>  T& operator>(vector<T>& v, size_t i) { ret v[i]; }template<typename T>  T& operator>(basic_string<T>& v, size_t i) { ret v[i]; }
#define eve %2==0
#define odd %2
#define dig -'0'
#define let -'a'
...
#if MANY_TESTS
signed main() { ios_base::sync_with_stdio(0), in.tie(0), out.tie(0);preSolve();int t = 1;in >> t;fo(i, t) { if (DBG) { out << YELLOW << " -- " << (i + 1) << " --\n" << RESET; }solve(t); } postSolve(); }
#else
